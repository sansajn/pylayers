...
class layer(layer_interface.layer):
	...
	def tiles_not_in_cache(self, visible_tile_range):
		...
		for t in visible_tile_list:
			if self.find_in_ram_cache(t[1], t[0], self.zoom) is None:
				if self.find_in_disk_cache(t[1], t[0], self.zoom) is None:
					tiles.append(t)
		return tiles
	...
	def visible_tiles(self):
		...
		if is_intersection_empty(rc_map, rc_view):
			...
			if x0 < 0:
				n = ( int(abs(x0)/w_t), min(int(math.ceil((abs(x0)+w)/float(w_t))),
					N-1)+1 )
			else:
				n = ( 0, min(int(math.ceil((w-x0)/float(w_t))), N-1)+1 )
			if y0 < 0:
				m = ( int(abs(y0)/h_t), min(int(math.ceil((abs(y0)+h)/float(h_t))),
					M-1)+1 )
			else:
