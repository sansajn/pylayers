...
import location_layer
import osm_layer
import debug_helper


class layers(QtGui.QWidget):
	'Sprostretkuvava manipulaciu vrstiev.'
	...
	def _open_file(self, fname):
		...
		if is_vof_file(str(fname)):
			...
			#self.add_layer(layer)
			pass
		elif is_location_file(str(fname)):
			layer = location_layer.layer(self)
			layer.create(str(fname))
			self.add_layer(layer)
		else:
...
def is_location_file(fname):
	return os.path.splitext(fname)[1] == '.loc'
